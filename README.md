...
- **Kanban** : `Devis`, `PAO`, `À imprimer` (BAT / En prod), `À facturer`, `Livré / Archivé` — groupes *à faire* / *en cours* (ou équivalent) selon la colonne.
- Glisser-déposer réel des cartes (HTML5 drag & drop), pastilles d'équipe multi-sélection ("Qui fait quoi").
- **Fiche détail** (modale, clic sur une carte, édition en ligne) : n° de devis, n° client, job, date, statut, équipe assignée, Planification (RDV client, job, pose ext./int., livraison, deadline), pièces jointes (logo, photo, devis, facture, BAT…), note libre, archivage/suppression.
...
- **Intégrations optionnelles** (voir `SETUP.md`) : synchro Google Agenda + Google Sheet à chaque modification, récap email quotidien à 8h30 (RDV/jobs/poses/livraisons/deadlines du jour), pièces jointes stockées sur Google Drive — via le compte crm.flashimpression@gmail.com.
